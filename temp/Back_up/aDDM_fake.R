...
aDDM_fake = function(x,nr.reps,fixation.model){
  ...
  # initialization of standard deviation in "cur.sd" which is passed to aevacc_4
  ...
  if (cur.set_size == 4){
    if (fixation.model == "Normal"){
      for (trial in trials){
        ...
        output = aevacc_4(max.sim.rep,max.dur,cur.sd,theta,ev.update[ev.update.pos.min:ev.update.pos.max],cur.fixations,cur.durations,fix.loc.sim,fix.dur.sim)
    ...
    if (fixation.model == "Random"){
      for (trial in trials){
        ...
        # Main function that spits out a vector with all choices and reaction times
        output = aevacc_ranfix_4(max.sim.rep,max.dur,cur.sd,theta,ev.update[ev.update.pos.min:ev.update.pos.max],fix.loc.sim,fix.dur.sim)
  ...
  if (cur.set_size == 6){
    if(fixation.model == "Normal"){
      for (trial in trials){
        ...
        # Main function that spits out a vector with all choices and reaction times
        output = aevacc_6(max.sim.rep,max.dur,cur.sd,theta,ev.update[ev.update.pos.min:ev.update.pos.max],cur.fixations,cur.durations,fix.loc.sim,fix.dur.sim)
    ...
    if(fixation.model == "Random"){
      for (trial in trials){
        ...
        # Main function that spits out a vector with all choices and reaction times
        output = aevacc_ranfix_6(max.sim.rep,max.dur,cur.sd,theta,ev.update[ev.update.pos.min:ev.update.pos.max],fix.loc.sim,fix.dur.sim)
  ...
  if (cur.set_size == 8){
    if(fixation.model == "Normal"){
      for (trial in trials){
        ...
        # Main function that spits out a vector with all choices and reaction times
        output = aevacc_8(max.sim.rep,max.dur,cur.sd,theta,ev.update[ev.update.pos.min:ev.update.pos.max],cur.fixations,cur.durations,fix.loc.sim,fix.dur.sim)
    ...
    if(fixation.model == "Random"){
      for (trial in trials){
        ...
        # Main function that spits out a vector with all choices and reaction times
        output = aevacc_ranfix_8(max.sim.rep,max.dur,cur.sd,theta,ev.update[ev.update.pos.min:ev.update.pos.max],fix.loc.sim,fix.dur.sim)
  ...
  ####### We are checking how many simulations were used in the generated data set --> to divide by the right number later in aDDM.choice.table.fake
  
  nr.reps.fake = max(aDDM.general.output.frame.fake$Repetition)
  
  aDDM.choice.table = ddply(aDDM.general.output.frame,.(Subject,Theta,Variance,Driftrate,Trial,Choice,RT_no_sacc.bins),
                           summarise,
                           Choice.percentage = (length(RT_no_sacc.bins))/nr.reps)
  ...
  aDDM.choice.table.fake = ddply(aDDM.general.output.frame.fake,.(Subject,Theta,Variance,Driftrate,Trial,Choice,RT_no_sacc.bins),
                                summarise,
                                Choice.percentage = (length(RT_no_sacc.bins))/nr.reps.fake)
