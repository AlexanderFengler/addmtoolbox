...
aDDM.full.output = function(x,nr.reps,fixation.model){
  ...
  if (cur.set_size == 4){
    if (fixation.model == "Normal"){
      for (trial in trials){
        ...
        output = aevacc_full_output_4(max.sim.rep,max.dur,cur.sd,theta,ev.update[ev.update.pos.min:ev.update.pos.max],cur.fixations,cur.durations,fix.loc.sim,fix.dur.sim)
  ...
  if (cur.set_size == 6){
    if (fixation.model == "Normal"){
      for (trial in trials){
        ...
        # Main function that spits out a vector with all choices and reaction times
        output = aevacc_full_output_6(max.sim.rep,max.dur,cur.sd,theta,ev.update[ev.update.pos.min:ev.update.pos.max],cur.fixations,cur.durations,fix.loc.sim,fix.dur.sim)
  ...
  if (cur.set_size == 8){
    if (fixation.model == "Normal"){
      for (trial in trials){
        ...
        # Main function that spits out a vector with all choices and reaction times
        output = aevacc_full_output_8(max.sim.rep,max.dur,cur.sd,theta,ev.update[ev.update.pos.min:ev.update.pos.max],cur.fixations,cur.durations,fix.loc.sim,fix.dur.sim)
