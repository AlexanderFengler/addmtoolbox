...
aDDM.ranfix.optimization.full.output = function(set.sizes,subjects){
  ...
  sourceCpp(file="Scripts/aDDM_Simulation/aevacc_ranfix_full_output_4.cpp")
  sourceCpp(file="Scripts/aDDM_Simulation/aevacc_ranfix_full_output_6.cpp")
  sourceCpp(file="Scripts/aDDM_Simulation/aevacc_ranfix_full_output_8.cpp")
  
  source('Scripts/aDDM_Simulation/aDDM_ranfix_full_output.R')
  ...
  sink("Sim_logs/log_aDDM_ranfix_full_output.txt",append=TRUE)
  
  out[[total_iteration_cnt]]= foreach (i = ita,.combine='rbind') %dopar% aDDM.ranfix.full.output(i)
