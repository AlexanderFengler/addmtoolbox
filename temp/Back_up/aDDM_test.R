...
aDDM.test = function(theta,nr.reps,fixation.model){
  ...
  cur.choices = choices[choices$Subject == 1 &  
                          choices$Set_size == 4 & 
                          choices$Trial_nr == 1, ]
  
  cur.choices$Snack_1 = 0
  cur.choices$Snack_2 = 1 
  cur.choices$Snack_3 = 2
  cur.choices$Snack_4 = 3
  ...
  if (cur.set_size == 4){
    if (fixation.model == "Random"){
      for (trial in trials){
        ...
        fix.dur.sim = rep(300,nr.fix.dur.sim)
